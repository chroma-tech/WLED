...
static uint8_t audioSyncEnabled =
    0; // bit field: bit 0 - send, bit 1 - receive (config value)
static bool udpSyncConnected =
    false; // UDP connection status -> true if connected to multicast group
...
static uint8_t soundAgc = 0; // Automagic gain control: 0 - none, 1 - normal, 2
                             // - vivid, 3 - lazy (config value)
#endif
// static float    volumeSmth = 0.0f;              // either sampleAvg or
// sampleAgc depending on soundAgc; smoothed sample
static float FFT_MajorPeak = 1.0f; // FFT: strongest (peak) frequency
static float FFT_Magnitude = 0.0f; // FFT: volume (magnitude) of peak frequency
static bool samplePeak =
    false; // Boolean flag for peak - used in effects. Responding routine may
           // reset this flag. Auto-reset after strip.getFrameTime()
static bool udpSamplePeak =
    false; // Boolean flag for peak. Set at the same time as samplePeak, but
           // reset by transmitAudioData
static unsigned long timeOfPeak = 0; // time of last sample peak detection.
static uint8_t fftResult[NUM_GEQ_CHANNELS] = {
    0}; // Our calculated freq. channel result table to be used by effects

// TODO: probably best not used by receive nodes
// static float agcSensitivity = 128;            // AGC sensitivity estimation,
// based on agc gain (multAgc). calculated by getSensitivity(). range 0..255
...
#endif
static uint16_t attackTime =
    80; // int: attack time in milliseconds. Default 0.08sec
static uint16_t decayTime =
    1400; // int: decay time in milliseconds.  Default 1.40sec

// peak detection
#ifdef ARDUINO_ARCH_ESP32
static void
detectSamplePeak(void); // peak detection function (needs scaled FFT results in
                        // vReal[]) - no used for 8266 receive-only mode
#endif
static void autoResetPeak(void); // peak auto-reset function
static uint8_t maxVol =
    31; // (was 10) Reasonable value for constant volume for 'peak detector', as
        // it won't always trigger  (deprecated)
static uint8_t binNum =
    8; // Used to select the bin for FFT based beat detection  (deprecated)
...
uint8_t soundSquelch =
    10; // squelch value for volume reactive routines (config value)
#else
uint8_t soundSquelch =
...
static uint8_t FFTScalingMode = 3; // 0 none; 1 optimized logarithmic; 2
                                   // optimized linear; 3 optimized square root
...
const double agcSampleDecay[AGC_NUM_PRESETS] = {
    0.9994f, 0.9985f, 0.9997f}; // decay factor for sampleMax, in case the
                                // current sample is below sampleMax
const float agcZoneLow[AGC_NUM_PRESETS] = {32, 28,
                                           36}; // low volume emergency zone
const float agcZoneHigh[AGC_NUM_PRESETS] = {240, 240,
                                            248}; // high volume emergency zone
const float agcZoneStop[AGC_NUM_PRESETS] = {
    336, 448, 304}; // disable AGC integrator if we get above this level
const float agcTarget0[AGC_NUM_PRESETS] = {
    112, 144, 164}; // first AGC setPoint -> between 40% and 65%
const float agcTarget0Up[AGC_NUM_PRESETS] = {
    88, 64, 116}; // setpoint switching value (a poor man's bang-bang)
const float agcTarget1[AGC_NUM_PRESETS] = {
    220, 224, 216}; // second AGC setPoint -> around 85%
const double agcFollowFast[AGC_NUM_PRESETS] = {
    1 / 192.f, 1 / 128.f, 1 / 256.f}; // quickly follow setpoint - ~0.15 sec
const double agcFollowSlow[AGC_NUM_PRESETS] = {
    1 / 6144.f, 1 / 4096.f, 1 / 8192.f}; // slowly follow setpoint  - ~2-15 secs
const double agcControlKp[AGC_NUM_PRESETS] = {
    0.6f, 1.5f, 0.65f}; // AGC - PI control, proportional gain parameter
const double agcControlKi[AGC_NUM_PRESETS] = {
    1.7f, 1.85f, 1.2f}; // AGC - PI control, integral gain parameter
const float agcSampleSmooth[AGC_NUM_PRESETS] = {
    1 / 12.f, 1 / 6.f,
    1 / 16.f}; // smoothing factor for sampleAgc (use rawSampleAgc if you want
               // the non-smoothed value)
// AGC presets end

static AudioSource *audioSource = nullptr;
static bool useBandPassFilter =
    false; // if true, enables a bandpass filter 80Hz-16Khz to remove noise.
           // Applies before FFT.
...
void FFTcode(void *parameter); // audio processing task: read samples, run FFT,
                               // fill GEQ channels from FFT results
static void
runMicFilter(uint16_t numSamples,
...
static float fftCalc[NUM_GEQ_CHANNELS] = {
    0.0f}; // Try and normalize fftBin values to a max of 4096, so that 4096/16
           // = 256.
static float fftAvg[NUM_GEQ_CHANNELS] = {
    0.0f}; // Calculated frequency channel results, with smoothing (used if
           // dynamics limiter is ON)
#ifdef SR_DEBUG
static float fftResultMax[NUM_GEQ_CHANNELS] = {
    0.0f}; // A table used for testing to determine how our post-processing is
           // working.
#endif

// audio source parameters and constant
constexpr SRate_t SAMPLE_RATE =
    22050; // Base sample rate in Hz - 22Khz is a standard rate. Physical sample
           // time -> 23ms
...
#define FFT_MIN_CYCLE                                                          \
  21 // minimum time before FFT task is repeated. Use with 22Khz sampling
...
constexpr uint16_t samplesFFT_2 =
    256; // meaningfull part of FFT results - only the "lower half" contains
         // useful information.
...
// results from FFT.
static float *vReal =
    nullptr; // FFT sample inputs / freq output -  these are our raw result bins
static float *vImag = nullptr; // imaginary parts

// Create FFT object
// lib_deps += https://github.com/kosme/arduinoFFT#develop @ 1.9.2
// these options actually cause slow-downs on all esp32 processors, don't use
// them. #define FFT_SPEED_OVER_PRECISION     // enables use of reciprocals (1/x
// etc) - not faster on ESP32 #define FFT_SQRT_APPROXIMATION       // enables
// "quake3" style inverse sqrt  - slower on ESP32 Below options are forcing
// ArduinoFFT to use sqrtf() instead of sqrt() #define sqrt_internal sqrtf //
// see https://github.com/kosme/arduinoFFT/pull/83 - since v2.0.0 this must be
// done in build_flags
...
void FFTcode(void *parameter) {
  DEBUGSR_PRINT("FFT started on core: ");
  DEBUGSR_PRINTLN(xPortGetCoreID());

  // allocate FFT buffers on first call
  if (vReal == nullptr)
    vReal = (float *)calloc(sizeof(float), samplesFFT);
  if (vImag == nullptr)
    vImag = (float *)calloc(sizeof(float), samplesFFT);
  ...
  ArduinoFFT<float> FFT =
      ArduinoFFT<float>(vReal, vImag, samplesFFT, SAMPLE_RATE, true);
  ...
  for (;;) {
    delay(
        1); // DO NOT DELETE THIS LINE! It is needed to give the IDLE(0) task
            // enough time and to keep the watchdog happy. taskYIELD(), yield(),
            // vTaskDelay() and esp_task_wdt_feed() didn't seem to work.
    ...
    if (disableSoundProcessing || (audioSyncEnabled & 0x02)) {
      vTaskDelayUntil(&xLastWakeTime,
                      xFrequency); // release CPU, and let I2S fill its buffers
      continue;
    }
    ...
    bool haveDoneFFT =
        false; // indicates if second measurement (FFT time) is valid
#endif
    ...
    if (start < esp_timer_get_time()) { // filter out overflows
      uint64_t sampleTimeInMillis = (esp_timer_get_time() - start + 5ULL) /
                                    10ULL; // "+5" to ensure proper rounding
    ...
    // release highest sample to volume reactive effects early - not strictly
    // necessary here - could also be done at the end of the function early
    // release allows the filters (getSample() and agcAvg()) to work with fresh
    // values - we will have matching gain and noise gate values when we want to
    // process the FFT results.
    micDataReal = maxSample;

#ifdef SR_DEBUG
    if (true) { // this allows measure FFT runtimes, as it disables the "only
                // when needed" optimization
#else
    if (sampleAvg > 0.25f) {  // noise gate open means that FFT results will be
                              // used. Don't run FFT if results are not needed.
#endif
      ...
      FFT.windowing(
          FFTWindow::Flat_top,
          FFTDirection::Forward); // Weigh data using "Flat Top" function -
                                  // better amplitude accuracy
      // FFT.windowing(FFTWindow::Blackman_Harris, FFTDirection::Forward);  //
      // Weigh data using "Blackman- Harris" window - sharp peaks due to
      // excellent sideband rejection
      FFT.compute(FFTDirection::Forward); // Compute FFT
      FFT.complexToMagnitude();           // Compute magnitudes
      vReal[0] =
          0; // The remaining DC offset on the signal produces a strong spike on
             // position 0 that should be eliminated to avoid issues.
      ...
      FFT_MajorPeak =
          constrain(FFT_MajorPeak, 1.0f,
                    11025.0f); // restrict value to range expected by effects

#if defined(WLED_DEBUG) || defined(SR_DEBUG)
      haveDoneFFT = true;
#endif

    } else { // noise gate closed - only clear results as FFT was skipped. MIC
             // samples are still valid when we do this.
    ...
    for (int i = 0; i < samplesFFT; i++) {
      float t = fabsf(vReal[i]); // just to be sure - values in fft bins should
                                 // be positive any way
      vReal[i] = t / 16.0f; // Reduce magnitude. Want end result to be scaled
                            // linear and ~4096 max.
    }                       // for()
    ...
    */                                    //  Range
      ...
      if (useBandPassFilter) {
        ...
        fftCalc[15] =
            fftAddAvg(165, 205) *
            0.75f; // 40 7106 - 8828 high             -- with some damping
      } else {
        ...
        // don't use the last bins from 216 to 255. They are usually
        // contaminated by aliasing (aka noise)
        fftCalc[15] =
            fftAddAvg(165, 215) *
            0.70f; // 50 7106 - 9259 high             -- with some damping
      }
      ...
      fftCalc[7] = fftAddAvg(
          19,
          26); // 7   818 - 1120 midrange -- 1Khz should always be the center !
      ...
      fftCalc[14] =
          fftAddAvg(104, 165) *
          0.88f; // 61 4479 - 7106 high mid + high  -- with slight damping
#endif
    ...
    // post-processing of frequency channels (pink noise adjustment, AGC,
    // smoothing, scaling)
    postProcessFFTResults((fabsf(sampleAvg) > 0.25f) ? true : false,
                          NUM_GEQ_CHANNELS);

#if defined(WLED_DEBUG) || defined(SR_DEBUG)
    if (haveDoneFFT && (start < esp_timer_get_time())) { // filter out overflows
      uint64_t fftTimeInMillis = ((esp_timer_get_time() - start) + 5ULL) /
                                 10ULL; // "+5" to ensure proper rounding
    ...
    if ((audioSource == nullptr) ||
        (audioSource->getType() !=
         AudioSource::Type_I2SAdc)) // the "delay trick" does not help for
                                    // analog ADC
#endif
      vTaskDelayUntil(&xLastWakeTime,
                      xFrequency); // release CPU, and let I2S fill its buffers
  ...
  for (int i = 0; i < numSamples; i++) {
    ...
    if (i < (numSamples - 1))
      ...
      highFilteredSample =
          ...
          beta2 * last_vals[1]; // special handling for last sample in array
    ...
    int numberOfChannels) // post-processing and post-amp of GEQ channels
{
  for (int i = 0; i < numberOfChannels; i++) {
    ...
    if (noiseGateOpen) { // noise gate open
      ...
      if (FFTScalingMode > 0)
        fftCalc[i] *=
            FFT_DOWNSCALE; // adjustment related to FFT windowing function
      // Manual linear adjustment of gain using sampleGain adjustment for
      // different input types.
      fftCalc[i] *=
          soundAgc ? multAgc
                   : ((float)sampleGain / 40.0f * (float)inputLevel / 128.0f +
                      1.0f / 16.0f); // apply gain, with inputLevel adjustment
      if (fftCalc[i] < 0)
        fftCalc[i] = 0;
    }
    ...
    if (fftCalc[i] > fftAvg[i]) // rise fast
      fftAvg[i] = fftCalc[i] * 0.75f +
                  0.25f * fftAvg[i]; // will need approx 2 cycles (50ms) for
                                     // converging against fftCalc[i]
    else {                           // fall slow
      if (decayTime < 1000)
        fftAvg[i] =
            fftCalc[i] * 0.22f +
            0.78f * fftAvg[i]; // approx  5 cycles (225ms) for falling to zero
      else if (decayTime < 2000)
        fftAvg[i] = fftCalc[i] * 0.17f +
                    0.83f * fftAvg[i]; // default - approx  9 cycles (225ms) for
                                       // falling to zero
      else if (decayTime < 3000)
        fftAvg[i] =
            fftCalc[i] * 0.14f +
            0.86f * fftAvg[i]; // approx 14 cycles (350ms) for falling to zero
      else
        fftAvg[i] =
            fftCalc[i] * 0.1f +
            0.9f * fftAvg[i]; // approx 20 cycles (500ms) for falling to zero
    }
    ...
    case 1:
      ...
      currentResult -=
          8.0f; // this skips the lowest row, giving some room for peaks
      if (currentResult > 1.0f)
        currentResult = logf(currentResult); // log to base "e", which is the
                                             // fastest log() function
      else
        currentResult =
            0.0f; // special handling, because log(1) = 0; log(0) = undefined
      currentResult *=
          0.85f + (float(i) / 18.0f); // extra up-scaling for high frequencies
      currentResult = mapf(currentResult, 0, LOG_256, 0,
                           255); // map [log(1) ... log(255)] to [0 ... 255]
      break;
    case 2:
      // Linear scaling
      ...
      if (currentResult < 1.0f)
        currentResult = 0.0f;
      currentResult *=
          0.85f + (float(i) / 1.8f); // extra up-scaling for high frequencies
      break;
    case 3:
      // square root scaling
      currentResult *= 0.38f;
      ...
      currentResult *=
          0.85f + (float(i) / 4.5f); // extra up-scaling for high frequencies
      currentResult = mapf(currentResult, 0.0, 16.0, 0.0,
                           255.0); // map [sqrt(1) ... sqrt(256)] to [0 ... 255]
      break;
...
// peak detection is called from FFT task when vReal[] contains valid FFT
// results
static void detectSamplePeak(void) {
  bool havePeak = false;
  // softhack007: this code continuously triggers while amplitude in the
  // selected bin is above a certain threshold. So it does not detect peaks - it
  // detects high activity in a frequency bin. Poor man's beat detection by
  // seeing if sample > Average + some value. This goes through ALL of the 255
  // bins - but ignores stupid settings Then we got a peak, else we don't. The
  // peak has to time out on its own in order to support UDP sound sync.
...
static void autoResetPeak(void) {
  ...
  if (millis() - timeOfPeak >
      peakDelay) { // Auto-reset of samplePeak after at least one complete frame
                   // has passed.
    samplePeak = false;
...
#define SR_DMTYPE 1   // default type = I2S
  ...
  audioSyncPacket {       // "packed" ensures that there are no additional gaps
    char header[6];       //  06 Bytes  offset 0
    uint8_t reserved1[2]; //  02 Bytes, offset 6  - gap required by the compiler
                          //  - not used yet
    float sampleRaw;      //  04 Bytes  offset 8  - either "sampleRaw" or
                          //  "rawSampleAgc" depending on soundAgc setting
    float sampleSmth;     //  04 Bytes  offset 12 - either "sampleAvg" or
                          //  "sampleAgc" depending on soundAgc setting
    uint8_t samplePeak; //  01 Bytes  offset 16 - 0 no peak; >=1 peak detected.
                        //  In future, this will also provide peak Magnitude
    uint8_t reserved2;  //  01 Bytes  offset 17 - for future extensions - not
                        //  used yet
    uint8_t fftResult[16]; //  16 Bytes  offset 18
    uint16_t reserved3; //  02 Bytes, offset 34 - gap required by the compiler -
                        //  not used yet
    float FFT_Magnitude; //  04 Bytes  offset 36
    float FFT_MajorPeak; //  04 Bytes  offset 40
  };

  // old "V1" audiosync struct - 83 Bytes payload, 88 bytes total (with padding
  // added by compiler) - for backwards compatibility
...
// set your config variables to their boot default value (this can also be done
// in readFromConfig() or a constructor if you prefer)
  ...
  double sampleMax =
      0.0; // Max sample over a few seconds. Needed for AGC controller.
  double micLev =
      0.0; // Used to convert returned value to have '0' as minimum. A leveller
  float expAdjF = 0.0f;    // Used for exponential filter.
  float sampleReal = 0.0f; // "sampleRaw" as float, to provide bits that are
                           // lost otherwise (before amplification by sampleGain
                           // or inputLevel). Needed for AGC.
  ...
  int16_t volumeRaw =
      0; // either sampleRaw or rawSampleAgc depending on soundAgc
  float my_magnitude = 0.0f; // FFT_Magnitude, scaled by multAgc

  // used to feed "Info" Page
  unsigned long last_UDPTime =
      0;                  // time of last valid UDP sound sync datapacket
  int receivedFormat = 0; // last received UDP sound sync format - 0=none, 1=v1
                          // (0.13.x), 2=v2 (0.14.x)
...
#define CYCLE_SAMPLEMAX 3500        // time window for merasuring
  ...
  void logAudio() {
    if (disableSoundProcessing &&
        (!udpSyncConnected || ((audioSyncEnabled & 0x02) == 0)))
      return; // no audio availeable
#ifdef MIC_LOGGER
    // Debugging functions for audio input and sound processing. Comment out the
    // values you want to see
    PLOT_PRINT("micReal:");
    PLOT_PRINT(micDataReal);
    PLOT_PRINT("\t");
    PLOT_PRINT("volumeSmth:");
    PLOT_PRINT(volumeSmth);
    PLOT_PRINT("\t");
    ...
    // PLOT_PRINT("samplePeak:");  PLOT_PRINT((samplePeak!=0) ? 128:0);
    // PLOT_PRINT("\t"); PLOT_PRINT("multAgc:");     PLOT_PRINT(multAgc, 4);
    // PLOT_PRINT("\t");
#endif
    PLOT_PRINTLN();
#endif
    ...
    //
    // Set true if wanting to see all the bands in their own vertical space on
    // the Serial Plotter, false if wanting to see values in Serial Monitor
    ...
    // [0..defaultScalingFromHighValue] to [0..scalingToHighValue], lower this
    // if you want to see smaller values easier
    ...
    // set higher if using scaleValuesFromCurrentMaxVal and you want a small
    // value that's also the current maxVal to look small on the plotter (can't
    // be 0 to avoid divide by zero error)
    ...
    if (printMaxVal) {
      PLOT_PRINTF("maxVal:%04d ",
                  maxVal + (mapValuesToPlotterSpace ? 16 * 256 : 0));
    }
    if (printMinVal) {
      PLOT_PRINTF("%04d:minVal ",
  ...
  void agcAvg(unsigned long the_time) {
    const int AGC_preset =
        (soundAgc > 0) ? (soundAgc - 1)
                       : 0; // make sure the _compiler_ knows this value will
                            // not change while we are inside the function
  ...
  void getSample() {
    ...
    const float weighting = 0.2f; // Exponential filter weighting. Will be
                                  // adjustable in a future release.
    const int AGC_preset =
        (soundAgc > 0) ? (soundAgc - 1)
                       : 0; // make sure the _compiler_ knows this value will
                            // not change while we are inside the function
...
#else
                            // this is the minimal code for reading analog mic
                            // input on 8266.
    // warning!! Absolutely experimental code. Audio on 8266 is still not
    // working. Expects a million follow-on problems.
    static unsigned long lastAnalogTime = 0;
    static float lastAnalogValue = 0.0f;
    if (millis() - lastAnalogTime > 20) {
      micDataReal =
          analogRead(A0); // read one sample with 10bit resolution. This is a
                          // dirty hack, supporting volumereactive effects only.
    ...
    if (micIn < micLev)
      micLev = ((micLev * 31.0f) + micDataReal) /
               32.0f; // align MicLev to lowest input signal
    ...
    if ((sampleMax < sampleReal) && (sampleReal > 0.5f)) {
      sampleMax =
          sampleMax +
          0.5f * (sampleReal - sampleMax); // new peak - with some filtering
      // another simple way to detect samplePeak - cannot detect beats, but
      // reacts on peak volume
      if (((binNum < 12) || ((maxVol < 1))) && (millis() - timeOfPeak > 80) &&
          (sampleAvg > 1)) {
        samplePeak = true;
        timeOfPeak = millis();
        udpSamplePeak = true;
      ...
      if ((multAgc * sampleMax > agcZoneStop[AGC_preset]) && (soundAgc > 0))
        sampleMax +=
            0.5f * (sampleReal - sampleMax); // over AGC Zone - get back quickly
      else
    ...
    sampleAvg = ((sampleAvg * 15.0f) + sampleAdj) /
                16.0f;            // Smooth it out over the last 16 samples.
  ...
  // Freqpixels, Freqwave, Gravfreq, (2D Swirl, 2D Waverly)
  void limitSampleDynamics(void) {
    const float bigChange =
    ...
    delta_time =
        constrain(delta_time, 1,
                  1000); // below 1ms -> 1ms; above 1sec -> sily lil hick-up
  ...
  void connectUDPSoundSync(void) {
    ...
    // necessary as we also want to transmit in "AP Mode", but the standard
    // "connected()" callback only reacts on STA connection
    static unsigned long last_connection_attempt = 0;

    if ((audioSyncPort <= 0) || ((audioSyncEnabled & 0x03) == 0))
      return; // Sound Sync not enabled
    if (udpSyncConnected)
      return; // already connected
    if (!(apActive || interfacesInited))
      return; // neither AP nor other connections availeable
    if (millis() - last_connection_attempt < 15000)
      return; // only try once in 15 seconds
    if (updateIsRunning)
      return;
  ...
  void transmitAudioData() {
    ...
    memset(reinterpret_cast<void *>(&transmitData), 0,
           sizeof(transmitData)); // make sure that the packet - including
                                  // "invisible" padding bytes added by the
                                  // compiler - is fully initialized
    ...
    if (fftUdp.beginMulticastPacket() !=
        0) { // beginMulticastPacket returns 0 in case of error
  ...
  void decodeAudioData(int packetSize, uint8_t *fftBuff) {
    ...
    memcpy(&receivedPacket, fftBuff,
           min((unsigned)packetSize,
               (unsigned)sizeof(receivedPacket))); // don't violate alignment -
                                                   // thanks @willmmiles#
  ...
  void decodeAudioData_v1(int packetSize, uint8_t *fftBuff) {
    audioSyncPacket_v1 *receivedPacket =
        reinterpret_cast<audioSyncPacket_v1 *>(fftBuff);
    // update samples for effects
    ...
    FFT_Magnitude = my_magnitude;
    FFT_MajorPeak =
        constrain(receivedPacket->FFT_MajorPeak, 1.0,
                  11025.0); // restrict value to range expected by effects
  }

  bool receiveAudioData() // check & process new data. return TRUE in case that
                          // new audio data was received.
  {
    if (!udpSyncConnected)
      return false;
...
#ifdef ARDUINO_ARCH_ESP32
    if ((packetSize > 0) &&
        ((packetSize < 5) || (packetSize > UDPSOUND_MAX_PACKET)))
      fftUdp.flush(); // discard invalid packets (too small or too big) - only
                      // works on esp32
#endif
    if ((packetSize > 5) && (packetSize <= UDPSOUND_MAX_PACKET)) {
      // DEBUGSR_PRINTLN("Received UDP Sync Packet");
      uint8_t fftBuff[UDPSOUND_MAX_PACKET + 1] = {
          0}; // fixed-size buffer for receiving (stack), to avoid heap
              // fragmentation caused by variable sized arrays
      fftUdp.read(fftBuff, packetSize);

      // VERIFY THAT THIS IS A COMPATIBLE PACKET
      if (packetSize == sizeof(audioSyncPacket) &&
          (isValidUdpSyncVersion((const char *)fftBuff))) {
        decodeAudioData(packetSize, fftBuff);
        // DEBUGSR_PRINTLN("Finished parsing UDP Sync Packet v2");
        haveFreshData = true;
        receivedFormat = 2;
      } else {
        if (packetSize == sizeof(audioSyncPacket_v1) &&
            (isValidUdpSyncVersion_v1((const char *)fftBuff))) {
...
public:
  ...
  void setup() override {
    ...
    if (!initDone) {
      ...
      // we will assign all usermod exportable data here as pointers to original
      // variables or arrays and allocate memory for pointers
      ...
      um_data->u_data[2] = fftResult; //*used (Blurz, DJ Light, Noisemove,
                                      // GEQ_base, 2D Funky Plank, Akemi)
      um_data->u_type[2] = UMT_BYTE_ARR;
      um_data->u_data[3] =
          &samplePeak; //*used (Puddlepeak, Ripplepeak, Waterfall)
      um_data->u_type[3] = UMT_BYTE;
      um_data->u_data[4] =
          &FFT_MajorPeak; //*used (Ripplepeak, Freqmap, Freqmatrix, Freqpixels,
                          // Freqwave, Gravfreq, Rocktaves, Waterfall)
      um_data->u_type[4] = UMT_FLOAT;
      um_data->u_data[5] = &my_magnitude; // used (New)
      um_data->u_type[5] = UMT_FLOAT;
      um_data->u_data[6] =
          &maxVol; // assigned in effect function from UI element!!!
                   // (Puddlepeak, Ripplepeak, Waterfall)
      um_data->u_type[6] = UMT_BYTE;
      um_data->u_data[7] =
          &binNum; // assigned in effect function from UI element!!!
                   // (Puddlepeak, Ripplepeak, Waterfall)
    ...
    i2s_driver_uninstall(I2S_NUM_0); // E (696) I2S: i2s_driver_uninstall(2006):
                                     // I2S port 0 has not installed
    ...
    if ((i2sckPin == I2S_PIN_NO_CHANGE) && (i2ssdPin >= 0) && (i2swsPin >= 0) &&
        ((dmType == 1) || (dmType == 4)))
      dmType = 5; // dummy user support: SCK == -1 --means--> PDM microphone
#endif

#if defined(CODEC_8316_ENABLE)
    DEBUGSR_PRINTLN(F("AR: Initializing ES8316 codec..."));
    bool codec_init = CodecES8316::Init(44, 43);
    if (!codec_init) {
      DEBUGSR_PRINTLN(F("AR: Failed to initialize ES8316 codec!"));
    }
    DEBUGSR_PRINTLN(F("AR: Codec initialized"));
#endif
    ...
    case 1:
      ...
      DEBUGSR_PRINTLN(F(I2S_MIC_CHANNEL_TEXT));
    ...
    case 5:
      ...
      DEBUGSR_PRINTLN(F(I2S_PDM_MIC_CHANNEL_TEXT));
      audioSource = new I2SSource(SAMPLE_RATE, BLOCK_SIZE, 1.0f / 4.0f);
      useBandPassFilter = true; // this reduces the noise floor on SPM1423 from
                                // 5% Vpp (~380) down to 0.05% Vpp (~5)
      delay(100);
      if (audioSource)
    ...
    default:
      ...
      delay(100);
      useBandPassFilter =
          true; // PDM bandpass filter seems to help for bad quality analog
      if (audioSource)
    ...
    if (!audioSource)
      enabled = false; // audio failed to initialise
#endif
    if (enabled)
      onUpdateBegin(false); // create FFT task, and initialize network

#ifdef ARDUINO_ARCH_ESP32
    if (FFT_Task == nullptr)
      enabled = false; // FFT task creation failed
    if ((!audioSource) ||
        (!audioSource
              ->isInitialized())) { // audio source failed to initialize. Still
                                    // stay "enabled", as there might be input
                                    // arriving via UDP Sound Sync
#ifdef WLED_DEBUG
      DEBUG_PRINTLN(F("AR: Failed to initialize sound input driver. Please "
                      "check input PIN settings."));
#else
      DEBUGSR_PRINTLN(F("AR: Failed to initialize sound input driver. Please "
                        "check input PIN settings."));
#endif
...
#endif
    if (enabled)
      disableSoundProcessing = false; // all good - enable audio processing
    if (enabled)
      connectUDPSoundSync();
    if (enabled && addPalettes)
      createAudioPalettes();
    initDone = true;
  }
...
#else
      udpSyncConnected = fftUdp.beginMulticast(
          WiFi.localIP(), IPAddress(239, 0, 0, 1), audioSyncPort);
#endif
    }
  }
  ...
  void loop() override {
    ...
    if (strip.isUpdating() && (millis() - lastUMRun < 2))
      return; // be nice, but not too nice

    // suspend local sound processing when "real time mode" is active (E131,
    // UDP, ADALIGHT, ARTNET)
    if ((realtimeOverride ==
         REALTIME_OVERRIDE_NONE) // please add other overrides here if needed
        && ((realtimeMode == REALTIME_MODE_GENERIC) ||
            (realtimeMode == REALTIME_MODE_E131) ||
            (realtimeMode == REALTIME_MODE_UDP) ||
            (realtimeMode == REALTIME_MODE_ADALIGHT) ||
            (realtimeMode ==
             REALTIME_MODE_ARTNET))) // please add other modes here if needed
    {
#if defined(ARDUINO_ARCH_ESP32) && defined(WLED_DEBUG)
      if ((disableSoundProcessing == false) &&
          (audioSyncEnabled == 0)) { // we just switched to "disabled"
        DEBUG_PRINTLN(F("[AR userLoop]  realtime mode active - audio "
                        "processing suspended."));
        DEBUG_PRINTF_P(
            PSTR("               RealtimeMode = %d; RealtimeOverride = %d\n"),
...
#if defined(ARDUINO_ARCH_ESP32) && defined(WLED_DEBUG)
      if ((disableSoundProcessing == true) && (audioSyncEnabled == 0) &&
          audioSource->isInitialized()) { // we just switched to "enabled"
        DEBUG_PRINTLN(F(
            "[AR userLoop]  realtime mode ended - audio processing resumed."));
        DEBUG_PRINTF_P(
            PSTR("               RealtimeMode = %d; RealtimeOverride = %d\n"),
            int(realtimeMode), int(realtimeOverride));
      }
#endif
      if ((disableSoundProcessing == true) && (audioSyncEnabled == 0))
        lastUMRun = millis(); // just left "realtime mode" - update timekeeping
      disableSoundProcessing = false;
    }

    if (audioSyncEnabled & 0x02)
      disableSoundProcessing =
          true; // make sure everything is disabled IF in audio Receive mode
    if (audioSyncEnabled & 0x01)
      disableSoundProcessing =
          false; // keep running audio IF we're in audio Transmit mode
#ifdef ARDUINO_ARCH_ESP32
    if (!audioSource->isInitialized())
      disableSoundProcessing = true; // no audio source

    // Only run the sampling code IF we're not in Receive mode or realtime mode
    if (!(audioSyncEnabled & 0x02) && !disableSoundProcessing) {
      if (soundAgc > AGC_NUM_PRESETS)
        soundAgc = 0; // make sure that AGC preset is valid (to avoid array
                      // bounds violation)
...
#ifdef WLED_DEBUG
        // complain when audio userloop has been delayed for long time.
        // Currently we need userloop running between 500 and 1500 times per
        // second. softhack007 disabled temporarily - avoid serial console spam
        // with MANY leds and low FPS
        // if ((userloopDelay > 65) && !disableSoundProcessing &&
        // (audioSyncEnabled == 0)) {
        //  DEBUG_PRINTF_P(PSTR("[AR userLoop] hiccup detected -> was inactive
        //  for last %d millis!\n"), userloopDelay);
        //}
#endif
      ...
      do {
        ...
        agcAvg(t_now -
               userloopDelay); // Calculated the PI adjusted value as sampleAvg
    ...
    if ((audioSyncEnabled & 0x02) && udpSyncConnected) {
      ...
      if (millis() - lastTime > delayMs) {
        ...
        if (have_new_sample)
          ...
          fftUdp.flush(); // Flush udp input buffers if we haven't read it -
                          // avoids hickups in receive mode. Does not work on
                          // 8266.
#endif
        lastTime = millis();
      }
      if (have_new_sample)
        ...
        volumeSmth = syncVolumeSmth; // restore originally received sample for
                                     // next run of dynamics limiter
      limitSampleDynamics(); // run dynamics limiter on received volumeSmth, to
                             // hide jumps and hickups
    }

#if defined(MIC_LOGGER) || defined(MIC_SAMPLING_LOG) ||                        \
    defined(FFT_SAMPLING_LOG)
    ...
    if ((millis() - sampleMaxTimer) > CYCLE_SAMPLEMAX) {
      ...
      maxSample5sec =
          ...
          0.85f * ((soundAgc)
                       ? sampleAgc
                       : sampleAvg); // reset, and start with some smoothing
      if (sampleAvg < 1)
        maxSample5sec = 0; // noise gate
    } else {
      if ((sampleAvg >= 1))
        maxSample5sec = fmaxf(maxSample5sec,
                              (soundAgc) ? rawSampleAgc
                                         : sampleRaw); // follow maximum volume
    }
#else // similar functionality for 8266 receive only - use VolumeSmth instead of
      // raw sample data
    if ((millis() - sampleMaxTimer) > CYCLE_SAMPLEMAX) {
      sampleMaxTimer = millis();
      maxSample5sec = (0.15 * maxSample5sec) +
                      0.85 * volumeSmth; // reset, and start with some smoothing
      if (volumeSmth < 1.0f)
        maxSample5sec = 0; // noise gate
      if (maxSample5sec < 0.0f)
        maxSample5sec = 0; // avoid negative values
    } else {
      if (volumeSmth >= 1.0f)
        maxSample5sec =
            fmaxf(maxSample5sec, volumeRaw); // follow maximum volume
    }
#endif
  ...
  void onUpdateBegin(bool init) override {
    ...
    if (init && FFT_Task) {
      delay(25); // give some time for I2S driver to finish sampling before we
                 // suspend it
      vTaskSuspend(
          FFT_Task); // update is about to begin, disable task to prevent crash
      ...
      if (FFT_Task) {
        ...
        xTaskCreateUniversal( // xTaskCreateUniversal also works on -S2 and -C3
                              // with single core
            FFTcode,          // Function to implement the task
            "FFT",            // Name of the task
            3592, // Stack size in words // 3592 leaves 800-1024 bytes of task
                  // stack free
  ...
  void onUpdateBegin(bool init) {
    ...
    if (enabled)
      disableSoundProcessing = init; // init = true means that OTA is just
                                     // starting --> don't process audio
    updateIsRunning = init;
  }
#endif
...
#ifdef ARDUINO_ARCH_ESP32
  /**
   * handleButton() can be used to override default button behaviour. Returning
   * true will prevent button working in a default way.
   */
  ...
  void addToJsonInfo(JsonObject &root) override {
    ...
    String uiDomString =
        F("<button class=\"btn btn-xs\" onclick=\"requestJson({");
    ...
    if (enabled) {
      ...
      // Input Level Slider
      if (disableSoundProcessing ==
          false) { // only show slider when audio processing is running
        if (soundAgc > 0) {
          infoArr = user.createNestedArray(
              F("GEQ Input Level")); // if AGC is on, this slider only affects
                                     // fftResult[] frequencies
        } else {
          infoArr = user.createNestedArray(F("Audio Input Level"));
        }
        uiDomString =
            F("<div class=\"slider\"><div class=\"sliderwrap il\"><input "
              "class=\"noslide\" onchange=\"requestJson({");
        uiDomString += FPSTR(_name);
        uiDomString += F(":{");
        uiDomString += FPSTR(_inputLvl);
        uiDomString +=
            F(":parseInt(this.value)}});\" oninput=\"updateTrail(this);\" "
              "max=255 min=0 type=\"range\" value=");
        uiDomString += inputLevel;
        uiDomString += F(
            " /><div class=\"sliderdisplay\"></div></div></div>"); //<output
                                                                   // class=\"sliderbubble\"></output>
        infoArr.add(uiDomString);
      }
#endif
      // The following can be used for troubleshooting user errors and is so not
      // enclosed in #ifdef WLED_DEBUG
      ...
      if (audioSyncEnabled & 0x02) {
        ...
        if (audioSource && (audioSource->isInitialized())) {
          ...
          if (maxSample5sec > 1.0f) {
            ...
            snprintf_P(myStringBuffer, 15, PSTR(" - peak %3d%%"),
                       int(my_usage));
      ...
      // AGC or manual Gain
      if ((soundAgc == 0) && (disableSoundProcessing == false) &&
          !(audioSyncEnabled & 0x02)) {
        infoArr = user.createNestedArray(F("Manual Gain"));
        float myGain =
            ((float)sampleGain / 40.0f * (float)inputLevel / 128.0f) +
            1.0f / 16.0f; // non-AGC gain from presets
        infoArr.add(roundf(myGain * 100.0f) / 100.0f);
        infoArr.add("x");
      }
      if (soundAgc && (disableSoundProcessing == false) &&
          !(audioSyncEnabled & 0x02)) {
      ...
      if (audioSyncEnabled) {
        if (audioSyncEnabled & 0x01) {
          ...
          if ((udpSyncConnected) && (millis() - lastTime < 2500))
            infoArr.add(F(" v2"));
        ...
        infoArr.add("off");
      if (audioSyncEnabled && !udpSyncConnected)
        infoArr.add(" <i>(unconnected)</i>");
      if (audioSyncEnabled && udpSyncConnected &&
          (millis() - last_UDPTime < 2500)) {
        if (receivedFormat == 1)
          infoArr.add(F(" v1"));
        if (receivedFormat == 2)
          infoArr.add(F(" v2"));
      ...
      if ((fftTime / 100) >=
          FFT_MIN_CYCLE) // FFT time over budget -> I2S buffer will overflow
        infoArr.add("<b style=\"color:red;\">! ms</b>");
      else if ((fftTime / 80 + sampleTime / 80) >=
               FFT_MIN_CYCLE) // FFT time >75% of budget -> risk of instability
  ...
  void readFromJsonState(JsonObject &root) override {
    ...
    if (root.containsKey(F("rmcpal")) && root[F("rmcpal")].as<bool>()) {
      // handle removal of custom palettes from JSON call so we don't break
      // things
      removeAudioPalettes();
    }
  }
  ...
  bool readFromConfig(JsonObject &root) override {
    ...
    defined(CONFIG_IDF_TARGET_ESP32C3) || defined(CONFIG_IDF_TARGET_ESP32S3)
    if (dmType == 0)
      dmType = SR_DMTYPE; // MCU does not support analog
#if defined(CONFIG_IDF_TARGET_ESP32S2) || defined(CONFIG_IDF_TARGET_ESP32C3)
    if (dmType == 5)
      dmType = SR_DMTYPE; // MCU does not support PDM
#endif
#endif
    ...
    configComplete &=
        getJsonValue(top[FPSTR(_config)][F("squelch")], soundSquelch);
    ...
    configComplete &=
        getJsonValue(top[FPSTR(_frequency)][F("scale")], FFTScalingMode);

    configComplete &=
        getJsonValue(top[FPSTR(_dynamics)][F("limiter")], limiterOn);
    configComplete &=
        getJsonValue(top[FPSTR(_dynamics)][F("rise")], attackTime);
    ...
    if (initDone) {
      // add/remove custom/audioreactive palettes
      if ((oldAddPalettes && !addPalettes) || (oldAddPalettes && !enabled))
        removeAudioPalettes();
      if ((addPalettes && !oldAddPalettes && enabled) ||
          (addPalettes && !oldEnabled && enabled))
        createAudioPalettes();
  ...
  void appendConfigData(Print &uiScript) override {
    uiScript.print(
        F("ux='AudioReactive';")); // ux = shortcut for Audioreactive - fingers
                                   // crossed that "ux" isn't already used as JS
                                   // var, html post parameter or css style
#ifdef ARDUINO_ARCH_ESP32
    uiScript.print(
        F("uxp=ux+':digitalmic:pin[]';")); // uxp = shortcut for
                                           // AudioReactive:digitalmic:pin[]
    ...
    uiScript.print(
        F("addInfo(ux+':dynamics:limiter',0,' On ');")); // 0 is field type, 1
                                                         // is actual field
    uiScript.print(F(
        "addInfo(ux+':dynamics:rise',1,'ms <i>(&#x266A; effects only)</i>');"));
    uiScript.print(F(
        "addInfo(ux+':dynamics:fall',1,'ms <i>(&#x266A; effects only)</i>');"));
...
#ifdef ARDUINO_ARCH_ESP32
    uiScript.print(F("addInfo(ux+':digitalmic:type',1,'<i>requires "
                     "reboot!</i>');")); // 0 is field type, 1 is actual field
    ...
    !defined(CONFIG_IDF_TARGET_ESP32C3) && !defined(CONFIG_IDF_TARGET_ESP32S3)
    uiScript.print(
        F("addInfo(uxp,3,'<i>only use -1, 0, 1 or 3</i>','I2S MCLK');"));
#else
  ...
  /*
   * handleOverlayDraw() is called just before every show() (LED strip update
   * frame) after effects have set the colors. Use this to blank out some LEDs
   * or set them to a different color regardless of the set effect mode.
...
void AudioReactive::removeAudioPalettes(void) {
  ...
  DEBUG_PRINT(F("Total # of palettes: "));
  DEBUG_PRINTLN(customPalettes.size());
}
...
CRGB AudioReactive::getCRGBForBand(int x, int pal) {
  ...
  case 2:
    b = map(x, 0, 255, 0,
            NUM_GEQ_CHANNELS /
                2); // convert palette position to lower half of freq band
    hsv = CHSV(fftResult[b], 255, x);
    hsv2rgb_rainbow(hsv, value); // convert to R,G,B
    break;
  case 1:
    b = map(x, 1, 255, 0,
            10); // convert palette position to lower half of freq band
    hsv =
        CHSV(fftResult[b], 255, map(fftResult[b], 0, 255, 30, 255)); // pick hue
    hsv2rgb_rainbow(hsv, value); // convert to R,G,B
    break;
  default:
    if (x == 1) {
...
void AudioReactive::fillAudioPalettes() {
  ...
  if (int(lastCustPalette) >= palettes)
    lastCustPalette -= palettes;
...
const char AudioReactive::UDP_SYNC_HEADER[] PROGMEM =
    "00002"; // new sync header version, as format no longer compatible with
             // previous structure
const char AudioReactive::UDP_SYNC_HEADER_v1[] PROGMEM =
    "00001"; // old sync header version - need to add backwards-compatibility
             // feature
